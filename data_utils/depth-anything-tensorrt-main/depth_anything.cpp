...
std::vector<float> DepthAnything::preprocess(cv::Mat& image)
{
    ...
    std::cout << "Time of resize: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
    ...
    std::cout << "Time of normalized: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
    return input_tensor;
}
...
cv::Mat DepthAnything::predict(cv::Mat& image)
{
    ...
    auto end = std::chrono::system_clock::now();
    std::cout << "Time of preprocess: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
    ...
    std::cout << "Time of predict: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
    start = std::chrono::system_clock::now();
    // Postprocessing
    cudaMemcpyAsync(depth_data, buffer[1], input_h * input_w * sizeof(float), cudaMemcpyDeviceToHost);

    end = std::chrono::system_clock::now();
    std::cout << "Time of post: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
    // Convert the entire depth_data vector to a CV_32FC1 Mat
    // cv::Mat depth_mat(input_h, input_w, CV_32FC1, depth_data);
    cv::Mat depth_mat(input_h, input_w, CV_8UC4, depth_data);
